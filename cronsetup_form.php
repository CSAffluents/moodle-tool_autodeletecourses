 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class tool_oldcoursesremoval_cron_setup_form extends moodleform {
    public function definition() {
        $mform = $this->_form;

        $mform->addElement('selectyesno', 'cronenabled',
                get_string('cronenabled', 'tool_oldcoursesremoval'));
        $mform->setType('cronenabled', PARAM_BOOL);

        $mform->addElement('select', 'starthour',
                get_string('cronstarthour', 'tool_oldcoursesremoval'), range(0, 23));
        $mform->setType('starthour', PARAM_INT);

        $mform->addElement('select', 'stophour',
                get_string('cronstophour', 'tool_oldcoursesremoval'),
        ...
        $mform->addElement('duration', 'procesingtime',
                get_string('cronprocesingtime', 'tool_oldcoursesremoval'));
